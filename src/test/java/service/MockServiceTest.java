...
public class MockServiceTest {
    ...
    public void shouldReturnVayneWhenAnyChampionName() {
        ...
        System.out.println("Champion : " + champion.getName());
        //->champion은 mock객체이기 때문에 실제로 값이 없다. 따라서 출력x
    ...
    @Test
    public void shouldSetMidPositionUseVerity() {
        Champion champion = mock(Champion.class);
        champion.setName("직스");
    ...
    public void sizeOfObject() {
        ...
        //mock객체여서 size가 출력되지 않는다. 실제로 존재하지 않으니까.
        //눈으로 실제로 값을 확인해보는 것은 어렵다
    ...
    public void shouldAtMostThreeTimesWhenChampionGetName() {
        ...
        //위에서 3번(이하) 실행되었기 때문에 Pass
    }

    // 4-3. champion 객체에서 이름을 저장하는 로직이 실행되지 않았으면 Pass 하는 로직을 작성하세요.
    @Test
    public void shouldNotInvocationForGetChampionName() {
        Champion champion = mock(Champion.class);
//        champion.setName("시비르");
        champion.setPosition("바텀");
        champion.setHasSkinCount(4);

        verify(champion, never()).setName(anyString()); //132줄 주석제거하면 오류 발생.

//        verify(champion, atMost(3)).setName();  //132줄 주석제거하면 138줄 정상작동.
    }
